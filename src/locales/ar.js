export const ar = {
  // Common
  loading: "جاري التحميل...",
  search: "بحث",
  search_placeholder: "ابحث عن المنتجات...",
  category: "الفئة",
  sort_by: "ترتيب حسب",
  default: "افتراضي",
  price_low_high: "السعر: من الأقل إلى الأعلى",
  price_high_low: "السعر: من الأعلى إلى الأقل",
  rating_high_low: "التقييم: من الأعلى إلى الأقل",
  product_count: "{count} منتج",
  price_range: "نطاق السعر",
  clear_filters: "مسح جميع الفلاتر",
  prev: "السابق",
  next: "التالي",
  page_info: "صفحة {current} من {total}",
  
  // Navbar
  home: "الرئيسية",
  products: "المنتجات",
  categories: "الفئات",
  cart: "السلة",
  profile: "الملف الشخصي",
  logout: "تسجيل الخروج",
  
  // Auth
  login: "تسجيل الدخول",
  register: "تسجيل",
  login_required: "تسجيل الدخول مطلوب",
  login_desc: "يرجى التسجيل أو تسجيل الدخول لمتابعة إضافة العناصر إلى سلة التسوق الخاصة بك.",
  continue_browsing: "متابعة التصفح",
  welcome_back: "مرحباً بعودتك",
  sign_in_account: "تسجيل الدخول إلى حساب ShopSphere الخاص بك",
  test_credentials: "بيانات الاختبار",
  email: "البريد الإلكتروني",
  password: "كلمة المرور",
  remember_me: "تذكرني",
  forgot_password: "نسيت كلمة المرور؟",
  sign_in: "تسجيل الدخول",
  or_continue_with: "أو تابع مع",
  google: "جوجل",
  github: "جيت هب",
  dont_have_account: "ليس لديك حساب؟",
  sign_up_now: "سجل الآن",
  create_account: "إنشاء حساب",
  join_shopsphere: "انضم إلى ShopSphere اليوم",
  full_name: "الاسم الكامل",
  confirm_password: "تأكيد كلمة المرور",
  agree_to: "أوافق على",
  terms_conditions: "الشروط والأحكام",
  and: "و",
  privacy_policy: "سياسة الخصوصية",
  or_sign_up_with: "أو سجل باستخدام",
  already_have_account: "هل لديك حساب بالفعل؟",
  sign_in_link: "تسجيل الدخول",
  
  // Home Page
  welcome_to_shopsphere: "مرحباً بك في ShopSphere",
  global_shopping_destination: "وجهتك العالمية للتسوق عبر الإنترنت",
  discover_amazing_products: "اكتشف منتجات رائعة وأسعار لا تقبل المنافسة وخدمة استثنائية. تسوق بثقة واستمتع بتجربة تسوق سلسة من راحة منزلك.",
  shop_now: "تسوق الآن",
  learn_more: "اعرف المزيد",
  about_us: "من نحن",
  about_us_desc: "نحن شغوفون بتقديم أفضل تجربة تسوق لك",
  our_mission: "مهمتنا",
  our_mission_desc: "تقديم منتجات عالية الجودة بأسعار تنافسية مع تقديم خدمة عملاء استثنائية وخلق تجربة تسوق سلسة.",
  our_vision: "رؤيتنا",
  our_vision_desc: "أن نصبح منصة التسوق عبر الإنترنت الأكثر ثقة وتفضيلاً، المعروفة بالابتكار والموثوقية ورضا العملاء.",
  our_values: "قيمنا",
  our_values_desc: "النزاهة والتركيز على العملاء والتميز في الجودة والابتكار المستمر يوجه كل ما نقوم به. ثقتك هي أثمن ما لدينا.",
  our_services: "خدماتنا",
  our_services_desc: "نقدم مجموعة شاملة من الخدمات لتحسين تجربة التسوق الخاصة بك",
  free_shipping: "شحن مجاني",
  free_shipping_desc: "استمتع بالشحن المجاني على جميع الطلبات. لا يوجد حد أدنى للشراء!",
  support_24_7: "دعم على مدار الساعة",
  support_24_7_desc: "فريق دعم العملاء لدينا متاح دائماً لمساعدتك في أي وقت.",
  secure_payment: "دفع آمن",
  secure_payment_desc: "تسوق بثقة باستخدام نظام الدفع الآمن والمشفر لدينا.",
  easy_returns: "إرجاع سهل",
  easy_returns_desc: "إرجاع خالٍ من المتاعب خلال 30 يوماً. رضاك مضمون!",
  our_trusted_vendors: "موردونا الموثوقون",
  trusted_vendors_desc: "نتعاون مع علامات تجارية أصلية لنقدم لك منتجات عالية الجودة",
  become_vendor: "هل تريد أن تصبح مورداً؟",
  become_vendor_desc: "انضم إلى شبكة الموردين الموثوقين لدينا واصل إلى ملايين العملاء في جميع أنحاء العالم. نحن نوفر المنصة، وأنت توفر التميز.",
  partner_with_us: "كن شريكاً معنا",
  ready_to_shop: "هل أنت مستعد للبدء في التسوق؟",
  explore_collection: "استكشف مجموعتنا الواسعة من المنتجات واعثر على ما تبحث عنه بالضبط",
  browse_products: "تصفح المنتجات",
  discover_our: "اكتشف مجموعتنا المختارة بعناية من المنتجات الرائعة",
  quick_navigation: "تصفح سريع",
  explore_sections: "استكشف أقسام متجرنا المختلفة",
  learn_about_mission: "تعرف على مهمتنا وقيمنا",
  discover_services: "اكتشف ما نقدمه",
  meet_partners: "تعرف على شركائنا الموثوقين",
  hero_alt: "تجربة تسوق عبر الإنترنت",
  Featured_Products: "المنتجات المميزة",
  
  // Products Page
  our_products: "منتجاتنا",
  no_products_found: "لم يتم العثور على منتجات",
  no_products_desc: "لم نتمكن من العثور على أي منتجات تطابق معايير البحث الخاصة بك. حاول تعديل الفلاتر أو مصطلحات البحث.",
  no_products_category: "لا توجد منتجات في هذه الفئة.",
  added_to_cart: "تمت الإضافة إلى السلة!",
  view_details: "عرض التفاصيل",
  failed_load_categories: "فشل في تحميل الفئات",
  failed_load_products: "فشل في تحميل المنتجات",
  add_to_cart: "اضافة الى سلة",
  off: "خصم",
  in_stock: "متوفر",
  out_of_stock: "غير متوفر",
  only_left: "بقي {count} فقط",
  view_product_details: "عرض تفاصيل المنتج",
  auto_playing: "تشغيل تلقائي",
  slider_info: "تتغير المنتجات تلقائياً كل 5 ثوانٍ • {current} من {total}",
  prev_product: "المنتج السابق",
  next_product: "المنتج التالي",
  go_to_slide: "انتقل إلى الشريحة {index}",
  
  // Cart Page
  shopping_cart: "سلة التسوق",
  clear_cart: "إفراغ السلة",
  remove: "إزالة",
  proceed_to_checkout: "متابعة الشراء",
  
  // Checkout Page
  checkout: "إتمام الشراء",
  personal_info: "المعلومات الشخصية",
  first_name: "الاسم الأول",
  last_name: "اسم العائلة",
  phone: "رقم الهاتف",
  shipping_address: "عنوان الشحن",
  street_address: "اسم الشارع",
  city: "المدينة",
  state: "الولاية / المقاطعة",
  zip_code: "الرمز البريدي",
  country: "الدولة",
  payment_info: "معلومات الدفع",
  card_number: "رقم البطاقة",
  cardholder_name: "اسم حامل البطاقة",
  expiry_date: "تاريخ الانتهاء",
  cvv: "رمز التحقق (CVV)",
  order_notes: "ملاحظات الطلب (اختياري)",
  order_notes_placeholder: "تعليمات توصيل خاصة، رسالة هدية، إلخ.",
  back_to_cart: "العودة إلى السلة",
  place_order: "تأكيد الطلب",
  processing: "جاري المعالجة...",
  order_summary: "ملخص الطلب",
  subtotal: "المجموع الفرعي",
  shipping: "الشحن",
  tax: "الضريبة",
  total: "الإجمالي",
  free: "مجاني",
  secure_checkout: "دفع آمن",
  secure_checkout_desc: "معلومات الدفع الخاصة بك مشفرة وآمنة",
  cart_empty: "سلة التسوق فارغة",
  add_products_msg: "أضف بعض المنتجات قبل إتمام الشراء!",
  continue_shopping: "متابعة التسوق",
  
  // Wishlist & Orders
  wishlist: "المفضلة",
  my_wishlist: "قائمة أمنياتي",
  empty_wishlist: "قائمة أمنياتك فارغة",
  empty_wishlist_desc: "احفظ العناصر التي تحبها في قائمة أمنياتك وراجعها في أي وقت.",
  start_shopping: "ابدأ التسوق",
  order_history: "سجل الطلبات",
  my_orders: "طلباتي",
  no_orders: "لم يتم العثور على طلبات",
  no_orders_desc: "لم تقم بتقديم أي طلبات بعد.",
  order_id: "رقم الطلب",
  date: "التاريخ",
  status: "الحالة",
  order_total: "الإجمالي",
  items: "عناصر",
  view_order: "عرض الطلب",
  order_status_pending: "قيد الانتظار",
  order_status_processing: "جاري المعالجة",
  order_status_shipped: "تم الشحن",
  order_status_delivered: "تم التوصيل",
  add_to_wishlist: "إضافة إلى القائمة المفضلة",
  remove_from_wishlist: "إزالة من القائمة المفضلة",

  // Footer
  about_shopsphere: "عن ShopSphere",
  about_footer_desc: "وجهتك الشاملة لجميع احتياجات التسوق الخاصة بك. نقدم منتجات عالية الجودة مع خدمة عملاء استثنائية وشحن سريع في جميع أنحاء العالم.",
  quick_links: "روابط سريعة",
  contact_us: "اتصل بنا",
  all_rights_reserved: "جميع الحقوق محفوظة",
  address: "شارع الوحدة 123، غزة، فلسطين",
  
  // Validation Errors
  required_field: "هذا الحقل مطلوب",
  invalid_email: "البريد الإلكتروني غير صالح",
  card_length_error: "رقم البطاقة يجب أن يكون 16 رقمًا",
  cvv_length_error: "رمز التحقق يجب أن يكون 3-4 أرقام",
  password_min_length: "كلمة المرور يجب أن تكون 6 أحرف على الأقل",
  name_min_length: "الاسم يجب أن يكون حرفين على الأقل",
  passwords_no_match: "كلمات المرور غير متطابقة",
  confirm_password_required: "يرجى تأكيد كلمة المرور",
  agree_terms_required: "يجب الموافقة على الشروط والأحكام",
  
  // Categories Page
  browse_categories: "تصفح الفئات",
  categories_subtitle: "استكشف مجموعتنا الواسعة من المنتجات عبر فئات مختلفة.",
  all_products: "جميع المنتجات",
  retry: "إعادة المحاولة",
  please_login_to_add_cart: "يرجى تسجيل الدخول لإضافة عناصر إلى السلة",
  
  // Product Info
  quantity_label: "الكمية:",
  you_save_amount: "وفرت ${amount} ({percent}% خصم)",
  free_shipping_threshold: "على الطلبات أكثر من $50",
  warranty: "الضمان",
  qty: "الكمية",
};
